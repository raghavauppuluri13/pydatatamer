...
using VarNumber =
    std::variant<bool, char, int8_t, uint8_t, int16_t, uint16_t, int32_t,
...
/**
 * @brief DataTamer uses a simple "flat" schema of key/value pairs (each pair is
 * a "field").
 */
struct Schema {
    uint64_t hash = 0;
...
//
// void(const std::string& name_field, const BufferSpan payload, const
// std::string& type_name)
//
...
template <typename T> inline T Deserialize(BufferSpan &buffer) {
    T var;
...
[[nodiscard]] inline uint64_t AddFieldToHash(const TypeField &field,
                                             uint64_t hash) {
    // https://stackoverflow.com/questions/2590677/how-do-i-combine-hash-values-in-c0x
    // std::cout << "hash: " << hash << std::endl;
    std::cout << "field: " << field.field_name << std::endl;
    ...
    combine(fnv1a, field.field_name);
    combine(type_hasher, field.type);
    if (field.type == BasicType::OTHER) {
        combine(fnv1a, field.type_name);
    }
...
inline Schema BuilSchemaFromText(const std::string &txt) {
    ...
    while (std::getline(ss, line)) {
        ...
        std::string str_right =
            line.substr(space_pos + 1, line.size() - (space_pos + 1));
        trimString(str_left);
        ...
        if (offset != std::string::npos && str_type->at(offset) == '[') {
            ...
            if (pos != offset + 1) {
                ...
                std::string number_string =
                    line.substr(offset + 1, pos - offset - 1);
                field.array_size =
                    static_cast<uint16_t>(std::stoi(number_string));
            }
        }
    ...
    std::cout << "Schema hash: " << schema.hash << std::endl;
    std::cout << "Declared schema hash: " << declared_schema << std::endl;
...
template <typename NumberCallback>
bool ParseSnapshotRecursive(
    const TypeField &field,
    const std::map<std::string, FieldsVector> &types_list, BufferSpan &buffer,
    const NumberCallback &callback_number, const std::string &prefix) {
    ...
    auto doParse = [&](const std::string &var_name) {
        if (field.type != BasicType::OTHER) {
            ...
            for (const auto &sub_field : fields) {
                ParseSnapshotRecursive(sub_field, types_list, buffer,
                                       callback_number, var_name);
            }
        }
    };
...
inline bool ParseSnapshot(const Schema &schema, SnapshotView snapshot,
    ...
    for (size_t i = 0; i < schema.fields.size(); i++) {
        ...
        if (GetBit(snapshot.active_mask, i)) {
            ParseSnapshotRecursive(field, schema.custom_types, buffer,
                                   callback_number, "");
        }
    }
    return true;

...
constexpr uint8_t Magic[] = {137, 77,          67, 65,
                             80,  SpecVersion, 13, 10}; // "\x89MCAP0\r\n"
...
struct MCAP_PUBLIC Footer {
    ...
    Footer(ByteOffset summaryStart, ByteOffset summaryOffsetStart)
        : summaryStart(summaryStart), summaryOffsetStart(summaryOffsetStart),
          summaryCrc(0) {}
};
...
struct MCAP_PUBLIC Schema {
    ...
    Schema(const std::string_view name, const std::string_view encoding,
...
struct MCAP_PUBLIC Channel {
    ...
    Channel(const std::string_view topic,
            const std::string_view messageEncoding, SchemaId schemaId,
            const KeyValueMap &metadata = {})
        : topic(topic), messageEncoding(messageEncoding), schemaId(schemaId),
          metadata(metadata) {}
};
...
struct MCAP_PUBLIC Message {
    ChannelId channelId;
    /**
     * @brief An optional sequence number. If non-zero, sequence numbers should
     * be unique per channel and increasing over time.
     */
    uint32_t sequence;
    /**
     * @brief Nanosecond timestamp when this message was recorded or received
     * for recording.
     */
    Timestamp logTime;
    /**
    ...
    uint64_t dataSize;
    /**
     * @brief A pointer to the message payload. For readers, this pointer is
     * only valid for the lifetime of an onMessage callback or before the
     * message iterator is advanced.
     */
    ...
    AttachmentIndex(const Attachment &attachment, ByteOffset fileOffset)
        : offset(fileOffset),
          length(9 +
                 ...
                 /* crc */ 4),
          logTime(attachment.logTime), createTime(attachment.createTime),
          dataSize(attachment.dataSize), name(attachment.name),
...
struct MCAP_PUBLIC RecordOffset {
    ...
    RecordOffset(ByteOffset offset_, ByteOffset chunkOffset_)
        : offset(offset_), chunkOffset(chunkOffset_) {}
...
struct MCAP_PUBLIC MessageView {
    ...
    MessageView(const Message &message, const ChannelPtr channel,
                const SchemaPtr schema, RecordOffset offset)
        : message(message), channel(channel), schema(schema),

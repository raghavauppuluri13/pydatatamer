...
enum struct ReadSummaryMethod {
    ...
    NoFallbackScan,
    /**
     * @brief If the Summary section is missing or incomplete, allow falling
     * back to reading the file sequentially to produce seeking indexes and
     * summary statistics.
     */
    AllowFallbackScan,
    /**
     * @brief Read the file sequentially from Header to DataEnd to produce
     * seeking indexes and summary statistics.
     */
    ForceScan,
};
...
struct MCAP_PUBLIC IReadable {
    ...
    /**
     * @brief This method is called by MCAP reader classes when they need to
     * read a portion of the file.
     *
     * @param output A pointer to a pointer to the buffer to write to. This
     * method is expected to either maintain an internal buffer, read data into
     * it, and update this pointer to point at the internal buffer, or update
     * this pointer to point directly at the source data if possible. The
     * pointer and data must remain valid and unmodified until the next call to
     * read().
     ...
     *   requested size if the end of the file is reached. The output pointer
     * must be readable from `output` to `output + size`. If the read fails,
     * this method should return 0.
     */
    virtual uint64_t read(std::byte **output, uint64_t offset,
                          uint64_t size) = 0;
};
...
class MCAP_PUBLIC ICompressedReader : public IReadable {
  public:
    ...
    /**
     * @brief Reset the reader state, clearing any internal buffers and state,
     * and initialize with new compressed data.
     *
     ...
     */
    virtual void reset(const std::byte *data, uint64_t size,
                       uint64_t uncompressedSize) = 0;
    /**
     * @brief Report the current status of decompression. A StatusCode other
     * than `StatusCode::Success` after `reset()` is called indicates the
     * decompression was not successful and the reader is in an invalid state.
     */
...
class MCAP_PUBLIC BufferReader final : public ICompressedReader {
  public:
    void reset(const std::byte *data, uint64_t size,
               uint64_t uncompressedSize) override;
    uint64_t read(std::byte **output, uint64_t offset, uint64_t size) override;
    uint64_t size() const override;
...
class MCAP_PUBLIC ZStdReader final : public ICompressedReader {
  public:
     ...
     * @param output The output vector. This will be resized to
     * `uncompressedSize` to fit the data, or 0 if the decompression encountered
     * an error.
     * @return Status
     */
...
class MCAP_PUBLIC LZ4Reader final : public ICompressedReader {
  public:
     ...
     * @return Status
     */
    Status decompressAll(const std::byte *data, uint64_t size,
                         uint64_t uncompressedSize, ByteArray *output);
    LZ4Reader();
...
struct MCAP_PUBLIC ReadMessageOptions {
  public:
    /**
     * @brief Only messages with log timestamps greater or equal to startTime
     * will be included.
     */
    Timestamp startTime = 0;
    /**
     * @brief Only messages with log timestamps less than endTime will be
     * included.
     */
    Timestamp endTime = MaxTime;
    /**
     * @brief If provided, `topicFilter` is called on all topics found in the
     * MCAP file. If `topicFilter` returns true for a given channel, messages
     * from that channel will be included. if not provided, messages from all
     ...
     * if readOrder == FileOrder, messages will be returned in the order they
     * appear in the MCAP file. if readOrder == LogTimeOrder, messages will be
     * returned in ascending log time order. if readOrder ==
     * ReverseLogTimeOrder, messages will be returned in descending log time
     * order.
     */
...
class MCAP_PUBLIC McapReader final {
  public:
    ...
    /**
     * @brief Opens an MCAP file for reading from an already constructed
     * IReadable implementation.
     *
     * @param reader An implementation of the IReader interface that provides
     * raw MCAP data.
     ...
     * @param endTime Optional end time in nanoseconds. Messages equal to or
     * after this time will not be returned.
     */
    LinearMessageView readMessages(Timestamp startTime = 0,
                                   Timestamp endTime = MaxTime);
    /**
     * @brief Returns an iterable view with `begin()` and `end()` methods for
     * iterating Messages in the MCAP file. If a non-zero `startTime` is
     ...
     *
     * This method is automatically used by `readMessages()`, and only needs to
     * be called directly if the caller is manually constructing an iterator.
     *
     * @param startTime Start time in nanoseconds.
     * @param endTime Optional end time in nanoseconds.
     * @return Start and end byte offsets.
     */
    std::pair<ByteOffset, ByteOffset>
    byteRange(Timestamp startTime, Timestamp endTime = MaxTime) const;

    /**
     * @brief Returns a pointer to the IReadable data source backing this
     * reader. Will return nullptr if the reader is not open.
     */
    ...
    /**
     * @brief Look up a Channel record by channel ID. If the Channel has not
     * been encountered yet or does not exist in the file, this will return
     * nullptr.
     *
    ...
    /**
     * @brief Returns all of the parsed MetadataIndex records. Call
     * `readSummary()` first to fully populate this data structure. The
     * multimap's keys are the `name` field from each indexed Metadata.
     */
    ...
    // and do not need to be called directly unless you are implementing your
    // own reader functionality or tests.

    static Status ReadRecord(IReadable &reader, uint64_t offset,
                             Record *record);
    static Status ReadFooter(IReadable &reader, uint64_t offset,
                             Footer *footer);
    ...
    static Status ParseMessageIndex(const Record &record,
    ...
    static Status ParseAttachmentIndex(const Record &record,
    ...
    static Status ParseMetadataIndex(const Record &record,
                                     MetadataIndex *metadataIndex);
    static Status ParseSummaryOffset(const Record &record,
                                     SummaryOffset *summaryOffset);
    static Status ParseDataEnd(const Record &record, DataEnd *dataEnd);

    /**
     * @brief Converts a compression string ("", "zstd", "lz4") to the
     * Compression enum.
     */
    static std::optional<Compression>
    ParseCompression(const std::string_view compression);
...
struct MCAP_PUBLIC RecordReader {
    ...
    RecordReader(IReadable &dataSource, ByteOffset startOffset,
                 ByteOffset endOffset = EndOffset);

    void reset(IReadable &dataSource, ByteOffset startOffset,
               ByteOffset endOffset);
...
struct MCAP_PUBLIC TypedRecordReader {
    ...
    std::function<void(const SchemaPtr, ByteOffset, std::optional<ByteOffset>)>
        onSchema;
    std::function<void(const ChannelPtr, ByteOffset, std::optional<ByteOffset>)>
        onChannel;
    std::function<void(const Message &, ByteOffset, std::optional<ByteOffset>)>
        onMessage;
    ...
    std::function<void(const Record &, ByteOffset, std::optional<ByteOffset>)>
        onUnknownRecord;
...
/**
 * @brief Uses message indices to read messages out of an MCAP in log time
 * order. The underlying MCAP must be chunked, with a summary section and
 * message indexes. The required McapWriterOptions are:
 *  - noChunking: false
...
struct MCAP_PUBLIC IndexedMessageReader {
  public:
     ...
     * @return false if no more messages are to be read. If there was some error
     * reading the MCAP, `status()` will return a non-Success status.
     */
    bool next();
...
struct MCAP_PUBLIC LinearMessageView {
    ...
    LinearMessageView(McapReader &mcapReader, ByteOffset dataStart,
                      ByteOffset dataEnd, Timestamp startTime,
                      Timestamp endTime, const ProblemCallback &onProblem);
    LinearMessageView(McapReader &mcapReader, const ReadMessageOptions &options,

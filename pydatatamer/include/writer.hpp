...
struct MCAP_PUBLIC McapWriterOptions {
    ...
    /**
     * @brief Do not write Summary or Summary Offset sections to the file,
     * placing the Footer record immediately after DataEnd. This can provide
     * some speed boost to file writing and produce smaller files, at the
     * expense of requiring a conversion process later if fast summarization or
     * indexed access is desired.
     */
     ...
     * compressed (if compression is enabled) and written to disk. Note that
     * smaller Chunks may be written, such as the last Chunk in the Data
     * section. This option is ignored if `noChunking=true`.
     */
    ...
    std::string profile;
    /**
     * @brief A freeform string written by recording libraries. For this
     * library, the default is "libmcap {Major}.{Minor}.{Patch}".
     */
...
class MCAP_PUBLIC IWritable {
  public:
    ...
    virtual void end() = 0;
    /**
     * @brief Returns the current size of the file in bytes. This must be equal
     * to the sum of all `size` parameters passed to `write()`.
     */
...
class MCAP_PUBLIC IChunkWriter : public IWritable {
  public:
    ...
    /**
     * @brief Returns the size in bytes of the compressed data. This will only
     * be called after `end()`.
     */
    virtual uint64_t compressedSize() const = 0;
    /**
     * @brief Returns true if `write()` has never been called since
     * initialization or the last call to `clear()`.
     */
...
class MCAP_PUBLIC McapWriter final {
  public:
     ...
     * pending writes. This should only be used in error cases as the output
     * MCAP file will be truncated.
     */
    void terminate();

    /**
     * @brief Add a new schema to the MCAP file and set `schema.id` to a
     * generated schema id. The schema id is used when adding channels to the
     * file.
     *
    ...
    /**
     * @brief Current MCAP file-level statistics. This is written as a
     * Statistics record in the Summary section of the MCAP file.
     */
    ...
    /**
     * @brief finishes the current chunk in progress and writes it to the file,
     * if a chunk is in progress.
     */
    void closeLastChunk();
    ...
    static uint64_t write(IWritable &output,
    ...
    static void write(IWritable &output, const KeyValueMap &map,
                      uint32_t size = 0);
